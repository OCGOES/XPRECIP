...
## 2. Data & Methods
The project’s initial direction was changed from producing a composite “mean” ERO for each SOM node to a more meaningful statistic-based analysis. While additional atmospheric variables were initially considered (e.g., CAPE, MSLP, TP, and other thermodynamic and kinematic variables), the scope of the project resided in focusing on a relatively smaller subset of variables.

The first step consisted of parsing through the ERO maps to create a .csv file of all days where a Slight Risk or higher exists in an area of the Eastern Contiguous United States (ECONUS), defined as east of the Continental Divide. This project only performs an analysis for the ECONUS, excluding the western third of the U.S. due to the dynamic differences, as well as to minimize terrain-induced forecasting problems that arise with the existing orography. Additionally, the timescale is set through the 2020-2025 period due to the availability of ERO data files. The WPC ERO shapefiles were then downloaded, reprojected to a common coordinate system, and rasterized. Due to the categorical nature of EROs, each risk level was assigned a numeric value (0 = None, 1 = Marginal, 2 = Slight, 3 = Moderate, 4 = High) and compiled into their own daily written NetCDF files. Several limitations arise regarding the ERO shapefile portion of this project; see “Potential Issues” section for more information.

Similarly, appropriate dynamical variables were downloaded for the same days as the EROs. To conduct a comprehensive analysis of the different generalized synoptic regimes that promote excessive rainfall, kinematic and moisture field variables were inputted into the machine learning model: 

- 500 hPa geopotential height (Z500)
- Total column water vapor (PWAT ERA5 equivalent)

It is important to note that all of the included derived variables are pre-processed on ERA5, which significantly cuts down on data acquisition and preprocessing time. The only processing post-acquisition consisted of renaming TCWV as PWAT, deriving geopotential height from geopotential (Z500 = Φ500/g0), and attributing the most-interpretable units. Additionally, the variables were standardized and merged into a singular NetCDF file to simplify ingesting them into the model. 

The chosen model for this study is self-organizing maps (SOM), which were trained on the z-score normalized Z500 and PWAT values to identify similar synoptic patterns and group them into discrete nodes. The SOM was tuned via adjusting hyperparameters (sigma, map size, learning rate, and number of iterations) where the performance was measured by the quantization error (QE). The hyperparameter configuration with the lowest QE became the chosen SOM, where both objective and subjective analyses were created. All of these products were created by grabbing the dates associated with the SOM node members, and grabbing the raw data or calculating new metrics specific to those pulled dates. Such subjective products include the mean synoptic regimes plotted, the frequency of exceedance for ERO occurrences, and the maximum categorical risk. More objective products include a seasonality analysis and the areal extent of ERO. Such products were created to support the overarching goal of holistically analyzing the relationships between synoptic mean flow patterns, moisture extent, and forecasted Excessive Rainfall Outlooks.
...
| Gather ERA5 data | 1 week | Great (4/4) | The group has worked with ERA5 data on TRITON and knows where to obtain it and how to use it. |
| Preprocess ERA5 data | 2 weeks | Fair (2/4) | The group has preprocessed ERA5 data before, but is not fully familiar with what format or formats are needed for machine learning. |
| Train the machine learning models | 3 weeks | Poor (1/4) | No group members have trained a machine learning model before, so it is difficult to anticipate how long this will take. |
